"""AnalyzeAgent Microservice — port 8011."""
import asyncio, sys
...
app = FastAPI(title="AnalyzeAgent", version="3.0.0")
...
@app.get("/health")
async def health():
    return {"status": "ok", "agent": "AnalyzeAgent", "port": 8011}

@app.post("/a2a")
async def a2a_endpoint(req: A2ARequest):
    try:
        from app.agent.sub_agents import AnalyzeAgent
        agent = AnalyzeAgent()
